// Package demo_v96_idempotency provides a mock write connector for v9.6 demos.
//
// CRITICAL: This mock connector does NOT move real money.
// It exists to demonstrate the v9.6 idempotency pipeline.
...
type MockWriteConnector struct {
	...
	usedKeys         map[string]string // idempotency_key -> receipt_id
...
// Implements idempotency: if same key is used, returns existing receipt.
func (c *MockWriteConnector) Execute(ctx context.Context, req write.ExecuteRequest) (*write.PaymentReceipt, error) {
	...
	// Check idempotency key - return existing receipt if already used
	c.mu.RLock()
	existingReceiptID, keyExists := c.usedKeys[req.IdempotencyKey]
	aborted := c.abortedEnvelopes[req.Envelope.EnvelopeID]
	c.mu.RUnlock()
	...
	// If idempotency key exists, create a duplicate receipt reference
	if keyExists && req.IdempotencyKey != "" {
		// Emit idempotency hit event
		if c.auditEmitter != nil {
			c.auditEmitter(events.Event{
				ID:             c.idGenerator(),
				...
				CircleID:       req.Envelope.ActorCircleID,
				IntersectionID: req.Envelope.IntersectionID,
				SubjectID:      existingReceiptID,
				SubjectType:    "receipt",
				Provider:       "mock-write",
				Metadata: map[string]string{
					"idempotency_key_prefix": req.IdempotencyKey[:16] + "...",
					"existing_receipt":       existingReceiptID,
					"reason":                 "idempotency_key_reused",
				},
			})
		}

		// Return a receipt referencing the original (simulated idempotent behavior)
		return &write.PaymentReceipt{
			ReceiptID:   existingReceiptID,
			EnvelopeID:  req.Envelope.EnvelopeID,
			ProviderRef: fmt.Sprintf("mock-%s-idempotent", existingReceiptID),
			...
			ProviderMetadata: map[string]string{
				...
				"simulated":        "true",
				"idempotent":       "true",
				"original_receipt": existingReceiptID,
			},
			Simulated: true, // CRITICAL: Mock never moves real money
		}, nil
	}
	...
	if c.auditEmitter != nil {
		c.auditEmitter(events.Event{
			...
			Metadata: map[string]string{
				...
				"currency":        req.Envelope.ActionSpec.Currency,
				"payee_id":        req.PayeeID,
				"idempotency_key": req.IdempotencyKey[:16] + "...",
			},
		})
	}
	...
	if req.IdempotencyKey != "" {
		c.mu.Lock()
		c.usedKeys[req.IdempotencyKey] = receiptID
		c.mu.Unlock()
	}
